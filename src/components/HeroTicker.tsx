...
export default function HeroTicker() {
  ...
  return (
    <div className="group mask-fade-x relative mt-20 w-full overflow-hidden">
      {!ready && <div className="mx-auto flex w-max gap-4">{renderItems()}</div>}
      {ready && (
        <>
          ...
          <div
            aria-hidden
            className="absolute top-0 left-0 flex w-max animate-marquee2 gap-4 motion-reduce:animate-none group-hover:[animation-play-state:paused]"
          >
            {renderItems()}
          </div>
        </>
